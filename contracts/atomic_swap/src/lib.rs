...
mod test {
    ...
    fn setup_zk_verifier(env: &Env, owner: &Address, listing_id: u64, leaf: &Bytes) -> Address {
    ...
    fn test_fee_deducted_and_sent_to_recipient() {
        ...
        let contract_id = env.register(AtomicSwap, ());
        let client = AtomicSwapClient::new(&env, &contract_id);
        client.initialize(&Address::generate(&env), &250u32, &fee_recipient, &60u64, &zk_verifier);
    ...
    fn test_zero_fee_bps_sends_full_amount_to_seller() {
        ...
        let contract_id = env.register(AtomicSwap, ());
        let client = AtomicSwapClient::new(&env, &contract_id);
        client.initialize(&Address::generate(&env), &0u32, &fee_recipient, &60u64, &zk_verifier);
            ...
            .any(|e| {
                if let soroban_sdk::xdr::ContractEventBody::V0(body) = &e.body {
                    body.topics.iter().any(|t| {
                        matches!(t, soroban_sdk::xdr::ScVal::Symbol(s) if s.0.as_slice() == b"contract_paused_event")
                    })
                } else {
                    false
                }
    ...
    fn test_unpause_emits_event() {
        ...
        let found = events
            ...
            .any(|e| {
                if let soroban_sdk::xdr::ContractEventBody::V0(body) = &e.body {
                    body.topics.iter().any(|t| {
                        matches!(t, soroban_sdk::xdr::ScVal::Symbol(s) if s.0.as_slice() == b"contract_unpaused_event")
                    })
                } else {
                    false
                }
    ...
    #[test]
    fn test_fee_set_at_init_applied_on_release() {
        let env = Env::default();
        env.mock_all_auths();
        let buyer = Address::generate(&env);
        let seller = Address::generate(&env);
        ...
        let client = AtomicSwapClient::new(&env, &contract_id);
        // 500 bps = 5%
        client.initialize(&Address::generate(&env), &500u32, &fee_recipient, &60u64, &zk_verifier);
        let swap_id = client.initiate_swap(
        ...
        assert_eq!(usdc_client.balance(&fee_recipient), 500);
    }
    ...
    fn test_get_swaps_by_buyer_multiple() {
        ...
        let buyer = Address::generate(&env);
        let seller = Address::generate(&env);
        let usdc_id = setup_usdc(&env, &buyer, 1500);
        ...
        let id1 = client.initiate_swap(&listing_id1, &buyer, &seller, &usdc_id, &500, &zk_id, &registry_id);
        let id2 = client.initiate_swap(&listing_id2, &buyer, &seller, &usdc_id, &500, &zk_id, &registry_id);
        let id3 = client.initiate_swap(&listing_id3, &buyer, &seller, &usdc_id, &500, &zk_id, &registry_id);
    ...
    fn test_get_swaps_by_buyer_page_empty_list() {
        ...
        let client = AtomicSwapClient::new(&env, &contract_id);
        let buyer = Address::generate(&env);
        // offset=0 on empty list should return empty page
    ...
    fn test_get_swaps_by_buyer_page_offset_at_end() {
        ...
        // offset == len returns empty page (not an error)
    ...
    #[test]
    #[should_panic(expected = "Error(Contract, #1)")]
    fn test_confirm_swap_rejects_empty_key_explicit() {
        let env = Env::default();
        env.mock_all_auths();
        let buyer = Address::generate(&env);
        let seller = Address::generate(&env);
        let (usdc_id, listing_id, registry_id, _cid, client, _admin) =
            setup_full(&env, &buyer, &seller, 500, 0);
        let swap_id = pending_swap(&env, &client, listing_id, &buyer, &seller, &usdc_id, &registry_id, 500);
        client.confirm_swap(&swap_id, &Bytes::new(&env));
    }

    #[test]
    fn test_decryption_key_stored_on_confirm() {
        let env = Env::default();
        env.mock_all_auths();
        let buyer = Address::generate(&env);
        let seller = Address::generate(&env);
        ...
        let usdc_id = setup_usdc(&env, &buyer, 1000);
        let (registry_id, listing_id) = setup_registry(&env, &seller, 0);
        let zk_id = setup_zk_verifier(&env, &seller, listing_id, &Bytes::from_slice(&env, b"key"));
        let contract_id = env.register(AtomicSwap, ());
        let client = AtomicSwapClient::new(&env, &contract_id);
    ...
    fn test_fee_floor_applies_for_small_amounts() {
        ...
        let usdc_client = token::Client::new(&env, &usdc_id);
        let (registry_id, listing_id) = setup_registry(&env, &seller, 0);
        let zk_id = setup_zk_verifier(&env, &seller, listing_id, &Bytes::from_slice(&env, b"k"));
        let contract_id = env.register(AtomicSwap, ());
        let client = AtomicSwapClient::new(&env, &contract_id);
        ...
        client.confirm_swap(&swap_id, &Bytes::from_slice(&env, b"k"));
        client.set_dispute_window(&10u32);
        env.ledger().with_mut(|li| li.sequence_number += 11);
        client.release_to_seller(&swap_id);
    ...
    /// Regression test: swap IDs must remain unique even after instance storage
    /// would have expired (counter is now in persistent storage).
    #[test]
    fn test_swap_ids_unique_across_many_swaps() {
        let env = Env::default();
        env.mock_all_auths();
        let buyer = Address::generate(&env);
        let seller = Address::generate(&env);

        // Use price 0 so each swap can reuse the same listing after cancellation.
        let (usdc_id, listing_id, registry_id, _cid, client, _admin) =
            setup_full(&env, &buyer, &seller, 0, 0);

        let n = 10u32;
        let mut seen_ids: soroban_sdk::Vec<u64> = soroban_sdk::Vec::new(&env);

        for i in 0..n {
            // Advance past cancel_delay so the previous swap can be cancelled.
            env.ledger().with_mut(|li| li.timestamp = li.timestamp.saturating_add(61));

            // Cancel the previous swap to free the listing for the next one.
            if i > 0 {
                let prev_id = seen_ids.get(i - 1).unwrap();
                client.cancel_swap(&prev_id);
            }

            // Mint fresh USDC for the buyer each round.
            token::StellarAssetClient::new(&env, &usdc_id).mint(&buyer, &500_i128);

            let swap_id =
                pending_swap(&env, &client, listing_id, &buyer, &seller, &usdc_id, &registry_id, 500);

            assert!(
                !seen_ids.contains(&swap_id),
                "duplicate swap ID detected: {}",
                swap_id
            );
            seen_ids.push_back(swap_id);
        }

        assert_eq!(seen_ids.len(), n);
    }
}
